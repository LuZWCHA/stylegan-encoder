...
def create_from_images(tfrecord_dir, image_dir, shuffle):
    ...
    img = np.asarray(PIL.Image.open(image_filenames[0]).convert('RGBA'))
    ...
    if channels not in [1, 4]:
        error('Input images must be stored as RGB or RGBA or grayscale')

    with TFRecordExporter(tfrecord_dir, len(image_filenames)) as tfr:
        order = tfr.choose_shuffled_order() if shuffle else np.arange(len(image_filenames))
        for idx in range(order.size):
            filename = image_filenames[order[idx]]
            img = np.asarray(PIL.Image.open(filename).convert('RGBA'))
            if channels == 1:
