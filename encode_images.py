...
def main():
    ...
    # Masking params
    #we don't need these params disable all
    ...
    parser.add_argument('--composite_blur', default=0, help='Size of blur filter to smoothly composite the images', type=int)
    ...
    for images_batch in tqdm(split_to_batches(ref_images, args.batch_size), total=len(ref_images)//args.batch_size):
        ...
        for loss_dict in pbar:
            ...
            if args.output_video and (vid_count % args.video_skip == 0):
              ...
              for i, name in enumerate(names):
                video_frame = PIL.Image.fromarray(batch_frames[i], 'RGBA').resize((args.video_size,args.video_size),PIL.Image.LANCZOS)
                video_out[name].write(cv2.cvtColor(np.array(video_frame).astype('uint8'), cv2.COLOR_RGBA2BGR))
        ...
        for img_array, dlatent, img_path, img_name in zip(generated_images, generated_dlatents, images_batch, names):
            mask_img = None
            #we don't need to add a mask so we disabled the maske compostition
