...
def G_synthesis(
    ...
    dlatent_size        = 512,          # Disentangled latent (W) dimensionality.
    num_channels        = 4,            # Number of output color channels.
    ...
    **_kwargs):                         # Ignore unrecognized keyword args.
    ...
    def torgba(res, x): # res = 2..resolution_log2
        lod = resolution_log2 - res
        with tf.variable_scope('ToRGBA_lod%d' % lod):
            return apply_bias(conv2d(x, fmaps=num_channels, kernel=1, gain=1, use_wscale=use_wscale))
    ...
    if structure == 'fixed':
        for res in range(3, resolution_log2 + 1):
            x = block(res, x)
        images_out = torgba(resolution_log2, x)

    # Linear structure: simple but inefficient.
    if structure == 'linear':
        images_out = torgba(2, x)
        for res in range(3, resolution_log2 + 1):
            lod = resolution_log2 - res
            x = block(res, x)
            img = torgba(res, x)
    ...
    if structure == 'recursive':
        ...
        def grow(x, res, lod):
            y = block(res, x)
            img = lambda: upscale2d(torgba(res, y), 2**lod)
            img = cset(img, (lod_in > lod), lambda: upscale2d(tflib.lerp(torgba(res, y), upscale2d(torgba(res - 1, x)), lod_in - lod), 2**lod))
