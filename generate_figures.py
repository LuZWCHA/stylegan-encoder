...
url_mcskins     = 'network-snapshot-001640.pkl'
...
def load_Gs(url):
    if url not in _Gs_cache:
        # dnnlib.util.open_url(url, cache_dir=config.cache_dir)
        cache_dir=config.models_dir
        import glob
        cache_files = glob.glob(os.path.join(cache_dir, url))
        if len(cache_files) == 1:
            f = open(cache_files[0], "rb")
...
def draw_uncurated_result_figure(png, Gs, cx, cy, cw, ch, rows, lods, seed):
    ...
    images = Gs.run(latents, None, **synthesis_kwargs) # [seed, y, x, rgba]

    canvas = PIL.Image.new('RGBA', (sum(cw // 2**lod for lod in lods), ch * rows))
    image_iter = iter(list(images))
    for col, lod in enumerate(lods):
        for row in range(rows * 2**lod):
            image = PIL.Image.fromarray(next(image_iter), 'RGBA')
            image = image.crop((cx, cy, cx + cw, cy + ch))
            image = image.resize((cw // 2**lod, ch // 2**lod), PIL.Image.ANTIALIAS)
            canvas.paste(image, (sum(cw // 2**lod for lod in lods[:col]), row * ch // 2**lod))
    canvas.save(png)

def draw_uncurated_result_figures(png, Gs, cx, cy, cw, ch, num, seed):
    print(png)
    latents = np.random.RandomState(seed).randn(num, Gs.input_shape[1])
    images = Gs.run(latents, None, **synthesis_kwargs) # [seed, y, x, rgba]

    imageList = list(images)
    image_iter = iter(imageList)

    i = 0
    for image in imageList:
        canvas = PIL.Image.new('RGBA', (cw, ch))
        image = PIL.Image.fromarray(image, 'RGBA')
        canvas.paste(image)
        canvas.save(png + '_' + str(i) + '.png')
        i += 1
...
def draw_style_mixing_figure(png, Gs, w, h, src_seeds, dst_seeds, style_ranges):
    ...
    canvas = PIL.Image.new('RGBA', (w * (len(src_seeds) + 1), h * (len(dst_seeds) + 1)))
    for col, src_image in enumerate(list(src_images)):
        canvas.paste(PIL.Image.fromarray(src_image, 'RGBA'), ((col + 1) * w, 0))
    for row, dst_image in enumerate(list(dst_images)):
        canvas.paste(PIL.Image.fromarray(dst_image, 'RGBA'), (0, (row + 1) * h))
        row_dlatents = np.stack([dst_dlatents[row]] * len(src_seeds))
        row_dlatents[:, style_ranges[row]] = src_dlatents[:, style_ranges[row]]
        row_images = Gs.components.synthesis.run(row_dlatents, randomize_noise=False, **synthesis_kwargs)
        for col, image in enumerate(list(row_images)):
            canvas.paste(PIL.Image.fromarray(image, 'RGBA'), ((col + 1) * w, (row + 1) * h))
    canvas.save(png)
...
def main():
    ...
    draw_uncurated_result_figure(os.path.join(config.result_dir, 'figure02-uncurated.png'), load_Gs(url_mcskins), cx=0, cy=0, cw=64, ch=64, rows=16, lods=[0,0,0,0,0,0], seed=5)
    draw_style_mixing_figure(os.path.join(config.result_dir, 'figure03-style-mixing.png'), load_Gs(url_mcskins), w=64, h=64, src_seeds=[639,701,687,615,2268], dst_seeds=[888,829,1898,1733,1614,845], style_ranges=[range(0,3)]*3+[range(3,6)]*2+[range(6,10)])
    draw_uncurated_result_figures(os.path.join(config.result_dir, 'figure01-uncurated'), load_Gs(url_mcskins), cx=0, cy=0, cw=64, ch=64, num=10, seed=3)
