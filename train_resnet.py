...
def generate_dataset_main(n=10000, save_path=None, seed=None, model_res=1024, image_size=256, minibatch_size=16, truncation=0.7):
    """
    Generates a dataset of 'n' images of shape ('size', 'size', 4) with random seed 'seed'
...
def get_resnet_model(save_path, model_res=64, image_size=256, depth=2, size=0, activation='elu', loss='logcosh', optimizer='adam'):
    # Build model
    ...
    x_init = None
    inp = Input(shape=(image_size, image_size, 4))

    #fliter 4 channels to 3channels
    x = Conv2D(3, 1, padding='same')(inp)
...
Model = 'D:\\Projects\\Python\\DL\\stylegan-encoder-master\\models\\network-snapshot-001640.pkl'
